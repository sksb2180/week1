...
template sudoku() {
    ...
    component range_puzzle[9][9];
    component range_solution[9][9];

    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {

            range_puzzle[i][j] = RangeProof(32);
            range_solution[i][j] = RangeProof(32);

            range_puzzle[i][j].in <== puzzle[i][j];
            range_puzzle[i][j].range[0] <== 0;
            range_puzzle[i][j].range[1] <== 9;

            range_solution[i][j].in <== solution[i][j];
            range_solution[i][j].range[0] <== 0;
            range_solution[i][j].range[1] <== 9;

            assert(range_puzzle[i][j].out == 1);
            assert(range_solution[i][j].out == 1);
